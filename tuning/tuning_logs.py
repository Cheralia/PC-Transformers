...
def log_trial_to_detailed_log(trials_path, trial, config, trial_time, avg_energy, write_header=False):
    ...
    with open(trials_path, "a") as f:
        if write_header:
            # Only tuned parameters + metrics
            f.write(f"{'Trial':<6} | {'Time(s)':<8} | {'Combined':<10} | {'Energy':<10} | {'Perplexity':<11} | "
                    f"{'n_embed':<7} | {'block_size':<10} | {'heads':<5} | {'blocks':<6} | {'T':<3} | {'Peak LR':<10}\n")
            f.write("-" * 120 + "\n")
        
        f.write(f"{trial.number:<6} | {trial_time:<8.1f} | {combined_loss:<10.5f} | {avg_energy:<10.4f} | {avg_perplexity:<11.2f} | "
                f"{config.n_embed:<7} | {config.block_size:<10} | {config.num_heads:<5} | {config.n_blocks:<6} | {config.T:<3} | "
                f"{config.peak_learning_rate:<10.2e}\n")
